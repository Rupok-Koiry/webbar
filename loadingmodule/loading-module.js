...
function create() {
  ...
  const getAppNameForDisplay = () => {
    ...
    return deviceInfo.browser.name === "Mobile Safari"
      ? ""
  ...
  const setRoot = (rootNode) => {
    ...
    camPermissionsRequest_ = document.getElementById(
      "requestingCameraPermissions"
    );
    camPermissionsFailedAndroid_ = document.getElementById(
      "cameraPermissionsErrorAndroid"
    );
    camPermissionsFailedApple_ = document.getElementById(
      "cameraPermissionsErrorApple"
    );
    micPermissionsFailedAndroid_ = document.getElementById(
      "microphonePermissionsErrorAndroid"
    );
    micPermissionsFailedApple_ = document.getElementById(
    ...
    cameraSelectionError_ = document.getElementById(
      "cameraSelectionWorldTrackingError"
    );
    motionPermissionsErrorApple_ = document.getElementById(
  ...
  const promptUserToChangeBrowserSettingsMicrophone = () => {
    ...
    if (ua.includes("Linux")) {
      ...
      micPermissionsFailedApple_.getElementsByClassName(
        "wk-app-name"
      )[0].innerText = getAppNameForDisplay();
    }
  };
  ...
  const promptUserToChangeBrowserSettingsCamera = () => {
    ...
    if (ua.includes("Linux")) {
      ...
      camPermissionsFailedApple_.getElementsByClassName(
        "wk-app-name"
      )[0].innerText = getAppNameForDisplay();
    }
  };
  ...
  const promptUserToChangeBrowserMotionSettings = () => {
    ...
    if (XR8.XrDevice.deviceEstimate().osVersion.startsWith("12")) {
      ...
      motionPermissionsErrorApple_.getElementsByClassName(
        "wk-app-name"
      )[0].innerText = getAppNameForDisplay();
    }
    hideLoadingScreen(false);
  ...
  needsCookie_ =
    isAndroid &&
    !document.cookie.includes("previouslyGotCameraPermission=true");
  ...
  const pipelineModule = () => ({
    ...
    onUpdate: ({ processCpuResult }) => {
      ...
      numUpdates_++;
      if (
        numUpdates_ > FIRST_FRAME_DELAY &&
        !waitingOnAppResources_ &&
        appLoaded_()
      ) {
        if (needsCookie_) {
          document.cookie =
    ...
    onException: (error) => {
      ...
      if (error instanceof Object) {
        ...
        if (error.type === "configuration") {
          if (
            error.source === "reality" &&
            error.err === "slam-front-camera-unsupported"
          ) {
